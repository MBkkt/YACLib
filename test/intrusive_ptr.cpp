...
size_t Base::sInstances = 0;

using Counter = container::Counter<Base>;
class X : public Base {};
...
GTEST_TEST(ctor, default) {
  ...
  EXPECT_TRUE(px == nullptr);
  //  EXPECT_TRUE(nullptr == px);
}

GTEST_TEST(ctor, pointer) {
  EXPECT_EQ(Base::sInstances, 0);
  {
    Ptr<Base> px{nullptr};
